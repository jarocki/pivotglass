...
class TestCharacterModeLlmProfileField:
    ...
    def test_llm_profile_default_is_none_for_all_modes(self):
        """All 9 modes except full_troll must have llm_profile=None (F62 preservation)."""
        static_modes = {name: mode for name, mode in DEFAULT_MODES.items() if name != "full_troll"}
        for name, mode in static_modes.items():
            assert mode.llm_profile is None, (
                f"Mode '{name}' has llm_profile set -- only full_troll should in C-1 "
                "(DEC-30-CHARACTER-V2-006: other 9 modes ship at llm_profile=None)"
            )
    ...
    def test_llm_profile_is_none_on_legacy_static_modes(self):
        """default and ninja modes must stay KEEP_STATIC (llm_profile=None)."""
        assert DEFAULT_MODES["default"].llm_profile is None
        assert DEFAULT_MODES["ninja"].llm_profile is None
...
class TestSetCharacterIntegration:
    ...
    def test_set_character_ninja_uses_v1_composition_verbatim(self, runner):
        """set_character with ninja (KEEP_STATIC, llm_profile=None) uses F62 path."""
        ninja = DEFAULT_MODES["ninja"]
        assert ninja.llm_profile is None
        runner.set_character(ninja)
        sys_content = runner.conversation[0]["content"]
        expected_prefix = f"Character mode: {ninja.name}\n{ninja.personality}\n\n"
        assert sys_content.startswith(expected_prefix)
