...
**Aggregate (reconciled 2026-05-29, v0.2.0 wave landing):** Phases 0–14 complete (v1 through stable `v0.1.0` shipped 2026-05-19; v1-hardening F59/F60/F62/F62B/F63/F64/F61 all landed by 2026-05-26). Phases 16 + 17 closed the v2 strategic scoping (2026-05-27). Phases 17B + 17C closed the v0.2.0 MVP parallel wave (M-1 dossier panel + C-1 full_troll persona, 2026-05-28). Phases 17D + 17E closed the v0.2.x continuation wave (M-2 extractors + `get_dossier_state` LLM tool + C-2 ninja persona, 2026-05-29). 21 phases now landed total. Next work: **M-3 Dossier Scoring + Score Event Re-tune** (W-68-M3-DOSSIER-SCORING per Phase 16 roadmap §M-3) — load-bearing v2 product-center change that gates M-4 persistence; introduces `DossierSlotFilled` + `DossierEvidenceConfidenceUpgraded` ScoreEvent subtypes and re-tunes per-IOC `MODULE_RUN_SCORED` to baseline 1.0 while preserving F62/F63 streak math.
...
| W-68-M3-DOSSIER-SCORING | **Recommended next workflow** — Dossier M-3: Scoring + Score Event Re-tune. Introduce `DossierSlotFilled` and `DossierEvidenceConfidenceUpgraded` `ScoreEvent` subtypes per `.claude/plans/dossier-reframe-v2-roadmap.md` §M-3. Re-tune per-IOC `MODULE_RUN_SCORED` events down to weight 1.0 baseline. Implement slot-weight × confidence × rarity scoring. Preserves F62 streak chain + F63 milestone catch-up. **Load-bearing v2 product-center change**; gates M-4 persistence (predictions are themselves scored). Touches `gamification/scoring.py` + new `dossier/scoring.py` aggregator. Forbidden shortcuts: NO env-var bypass, NO "old scoring fallback" flag, NO new `ScoreEvent` emission outside `ScoringEngine` (single-authority). Planner re-stage required to author Evaluation Contract + Scope Manifest under successor workflow `w-68-m3-dossier-scoring`. | source + tests | _not started; planner re-stage_ | planned |

> **Recommended next work item:** `W-68-M3-DOSSIER-SCORING` — Dossier M-3 Scoring per Phase 16 roadmap §M-3. Implementer territory: `gamification/scoring.py` + new `dossier/scoring.py` aggregator. Authority preserved: F62 streak chain + F63 milestone catch-up + Sacred Practice 12 (`ScoringEngine` single-authority). Canonical chain: planner → guardian (provision) → implementer → reviewer → guardian (land). See `.claude/plans/dossier-reframe-v2-roadmap.md` §M-3 for binding scope, sequencing relative to M-4 persistence, and removal targets (no deprecation shim for old per-IOC weights).
>
