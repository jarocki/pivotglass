...
**Status:** in-progress (planner-staged 2026-06-09; implementer slice `wi-68-m9-impl-01` to follow). This section authored in the M-9 worktree by the M-9 planner; implementer commits per AP #74 orphan-prevention pattern in the same commit as source.
...
## Active Phase Pointer (2026-06-09)

**Phase Active (2026-06-09 — C-4 closed v2 character roadmap; M-9 planner-staged; v0.4.x dossier surface opens):** `W-68-M9-CROWDSOURCED-DOSSIERS` (Phase 17N — M-9 Crowdsourced Dossier Comparison + Public Actor Library, resolves DEC-68-DOSSIER-REFRAME-009 7-week-latency call on the Original Intent crowdsourcing axis). Phase 17M closed 2026-06-09 (merge `9a6a550`, impl `0d1f227`) — C-4 `columbo` live; v2 character roadmap CLOSED with final disposition 6 UPGRADE (full_troll/ninja/sun_tzu/bruce_lee/bureaucrat/columbo) + 4 KEEP_STATIC terminal (default/drunken_master/chuck_norris/bobby_hill); no C-5. M-9 planner-staged 2026-06-09; implementer slice `wi-68-m9-impl-01` is the next canonical dispatch. Worktree at `/Users/jarocki/src/ap/.worktrees/feature-68-m9-crowdsourced-dossiers` (base merge `9a6a550` — C-4 head). M-9 delivers four co-shipped sub-capabilities behind one feature branch: NEW `dossier/export.py` (STIX 2.1 bundle export consuming M-4 DossierState + M-4 PredictionsLog + M-5 AnalystNote rows + local opt-in `~/.ap/dossier_library/` helpers); NEW `dossier/import_.py` (read-only `ImportedDossier` value object — DEC-M9-IMPORT-READONLY-001); NEW `dossier/comparison.py` (`compare_dossiers` pure function + `ComparisonReport` dataclass — slot-by-slot diff, weighted completion per `SLOT_WEIGHTS`, validated-prediction ratio, unique-slot-fills per side); two NEW LLM tools (`export_dossier`, `compare_dossier`) — tool count 28 → 30; NEW `do_dossier` cmd2 command + extended chat `dossier` meta-command. Local opt-in library: `~/.ap/dossier_library/<actor_identifier>.json` (mode 0o700 per DEC-M9-LIBRARY-LOCATION-001; consent gate `AP_DOSSIER_PUBLISH=on` per DEC-M9-LIBRARY-OPTIN-001; path override `AP_DOSSIER_LIBRARY` for tests / power users). Reuses existing `core/graph.py::export_stix_bundle` python-stix2 round-trip (DEC-59-STIX-PROVENANCE-005). NO new ScoreEvent (DEC-M9-NO-EVENT-001 — `_DOSSIER_ACTIONS` unchanged at 4-tuple); NO new badge (DEC-M9-NO-NEW-BADGE-001); NO PII redaction (DEC-M9-PRIVACY-001 — privacy contract surfaced at consent boundary); NO network/upload (v1 Non-Goal "Federation" continues to bind). `core/workspace.py` / `models/database.py` / `models/stix.py` / every existing `dossier/*.py` EXCEPT `__init__.py` (exports only) / every `gamification/*.py` / `agent/runner.py` / `pyproject.toml` BYTEWISE UNCHANGED. F59/F60/F62/F63/F64 + Sacred Practice 5 + Sacred Practice 12 invariants preserved by construction. ~74 new tests across 5 NEW test files + 3-site update in `tests/test_agent_tools.py`. Runtime hygiene backlog (#49 / #50 / #51) remains opportunistic. Canonical chain `planner → guardian (provision) → implementer → reviewer → guardian (land)`. This pointer line is positioned as the last `**Phase ...` boldline in the document so `~/.claude/hooks/context-lib.sh:88` `get_plan_status()` tail-grep on `^#.*phase|^**Phase` resolves to current work instead of the historical `**Phase 6 rationale (new):**` narrative line in the Implementation Order section.
