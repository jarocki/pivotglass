# Pivotglass development history

> **Current state (2026-08-01):** Pivotglass v0.7.0 is the early-availability
> release. Bare `ap` opens the local browser interface; `ap tui` and `ap chat`
> open the terminal interface; `ap basic` and `ap repl` open the direct
> module-control console. Current operator guidance lives in `README.md`,
> `docs/QUICKSTART.md`, and `docs/USER_GUIDE.md`.
>
> This document preserves the project's development history. Dated plans,
> non-goals, active-phase pointers, private research paths, former names, and
> version targets below describe the context in which decisions were made.
> They are not current product promises unless a current guide repeats them.
...
**Current implementation checkpoint (2026-08-01):** v0.7.0 is released. The
...
catalog contains 14 CTI/OSINT modules, 29 LLM tools, and seven public character modes.
...
The Investigation Constellation, deterministic visualization-intent authority,
force-directed relationship graph, configuration center, and model controls
are implemented. Older “active,” “next,” and
...
**Target:** 1.0 -- a trustworthy multi-platform investigation framework with
the local Pivotglass browser interface as the primary experience, a
keyboard-first terminal equivalent, and the cmd2 console as direct control.
...
|-------|------|--------------------------|
| `ap` / `ap web` | **Primary interface.** Local browser workspace for evidence, coverage, relationships, configuration, and reports. | Investigation work that benefits from persistent visual context. |
| `ap tui` / `ap chat` | Keyboard-first terminal interface over the shared command and evidence authorities. | Terminal workflows and complete session scrollback. |
| `ap basic` / `ap repl` | Direct, deterministic `use → set → run` control over individual modules. | Explicit module selection, macros, and one-shot runs. |
...
## Historical v1 non-goals (superseded where noted)

This section records the scope before the browser interface became the primary
product surface. In particular, the former web-application non-goal is
superseded by the shipped Pivotglass interface.

These are explicitly out of scope for v1. They may appear in future versions but will not influence v1 design decisions:

- **Web application or GUI** — superseded; the local Pivotglass interface now ships
...
- **Required evidence:** 8 git-diff captures (modes.py + test_character_v2.py bounded by allowed_paths; runner.py + tools.py + streak.py + 7 gamification files + dossier/ MUST all be empty); 4 evidence files in `tmp/evidence-c3-philosophy-bureaucrat/` (3 ap-chat persona captures + token-budget check output + tool-count audit).
...
- **Required real-path checks:** 15 grep + python audits — 4 `hasattr` checks on the new `WorkspaceManager` methods; 1 in-process store→clear→empty roundtrip; 5 grep checks (`def clear`, `def _workspace_clear`, `workspace clear` in chat, `db_status` in chat, `_render_db_status_table` cross-file); 3 grep checks for DEC-WORKSPACE-DB-001/002/003 in MASTER_PLAN.md; 1 grep for `Phase 17P` (≥ 2 hits — table row + closeout section); 1 verification that the `clear()` body contains 6 post-commit count==0 RuntimeError checks (DEC-WORKSPACE-DB-007); 1 `git diff main --name-only` equals the 7-file allowed/required set.
