...
**Status:** implemented (2026-06-11); reviewer next. Implementer slice `wi-error-routing-impl-01` complete: catalog extended (HTTPStatusError 401/403 → API key; 429 → Rate limit; 5xx → Service; 4xx → Network); execute_tool except branch rewired; ToolContext.console injected; chat.py console propagated; 10 new tests pass; full suite +10 over baseline (2618 passed vs 2608 on main); runner.py / modules / forbidden paths BYTEWISE UNCHANGED per DEC-ERROR-ROUTING-001..007. Implementation commit SHA: TBD-post-commit (fixup commit follows per AP #74 pattern).
...
### Current Active Phase Pointer (2026-06-11)
