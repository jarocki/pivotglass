...
const instrumentWave = (instrument: Instrument): OscillatorType =>
  ["piano", "harp", "timpani", "taiko", "frame-drum", "woodblock"].includes(instrument) ? "triangle" : "sine";
const voice = (
...
export const SCORE_BIBLES: Record<string, ThemeScore> = {
  ...
  chuck_norris:{identity:"Chuck Norris",root:38,bpm:132,meter:4,phraseBars:4,scale:[0,2,4,5,7,9,10],motif:[0,4,3,5,2],answer:[5,2,4,1,0],cadence:[5,2,0],rhythm:[.5,.5,1,.5,1.5],chords:[[0,2,4],[3,5,1],[4,6,2],[0,2,4]],progression:[0,2,1,0],pulse:[1,0,1,0,0,1,0,0],form:[.58,.78,1,.52],strategy:"stop-time",voices:voice("french-horn","low-brass","strings","timpani",.9,{lead:{attack:.045,release:.4,pan:.22,gain:.078},pulse:{gain:.052,pan:-.25},pad:{gain:.042}})},
  full_troll:{identity:"Troll",root:43,bpm:118,meter:7,phraseBars:2,scale:[0,2,4,6,7,9,10],motif:[0,3,1,4,2,1],answer:[5,2,4,1,0],cadence:[6,1,0],rhythm:[.5,.5,1,.5,.5,1],chords:[[0,2,4],[3,5,1],[1,4,0],[0,2,4]],progression:[0,2,1,0],pulse:[1,0,1,0,1,0,0],form:[.52,.84,.66,.56],strategy:"crooked",voices:voice("bassoon","cello","strings","woodblock",1.05,{counter:{instrument:"clarinet",attack:.025,release:.28,pan:.5},pulse:{pan:-.5},air:{instrument:"bowed-cymbal",gain:.009}})},
  ...
  neuromancer:{identity:"Neuromancer",root:31,bpm:132,meter:4,phraseBars:4,scale:[0,2,3,5,7,8,10],motif:[0,0,4,3,6,5,3,2],answer:[6,3,5,2,4,1,2,0],cadence:[5,2,0],rhythm:[.5,.5,.5,.5,1,.5,.5,2],chords:[[0,2,4],[5,0,2],[3,5,1],[4,1,3]],progression:[0,1,2,0,3,2],pulse:[1,0,1,0,1,0,1,0],form:[.5,.74,1,.78],strategy:"signals",voices:voice("electric-cello","cello","strings","timpani",.68,{pad:{attack:.8,release:2.1,pan:-.45,filter:1450},bass:{attack:.018,release:.36,gain:.145,filter:820},lead:{attack:.035,release:.38,filter:3100},counter:{instrument:"electric-cello",pan:.6,filter:2600,gain:.034},pulse:{pan:-.25,gain:.058,filter:980},air:{instrument:"bowed-cymbal",gain:.016,pan:.68}})},
  the_matrix:{identity:"The Matrix",root:36,bpm:126,meter:4,phraseBars:4,scale:[0,1,3,5,7,8,10],motif:[0,0,4,2,0,5,4,2],answer:[6,3,1,4,2,0],cadence:[4,1,0],rhythm:[.5,.5,.5,.5,1,.5,.5,2],chords:[[0,2,4],[4,1,3],[3,5,1],[0,2,4]],progression:[0,1,2,1],pulse:[1,0,1,0,1,0,0,1],form:[.46,.7,.98,.64],strategy:"pursuit",voices:voice("solo-violin","cello","low-brass","taiko",.92,{lead:{attack:.035,release:.38,pan:.22},counter:{instrument:"strings",pan:-.65},pulse:{gain:.048,pan:.55},pad:{pan:-.32}})},
};
...
export class FlowMusicEngine {
  ...
  private active=new Set<AudioScheduledSourceNode>(); private stopped=false; private baseSeed:number;
  ...
  constructor(character:string,volume:number,seed?:number){
    ...
    const comp=this.context.createDynamicsCompressor();comp.threshold.value=-20;comp.knee.value=16;comp.ratio.value=6;comp.attack.value=.008;comp.release.value=.25;
  ...
  start(){if(this.timer||this.stopped)return;void this.context.resume();this.origin=this.context.currentTime+.08;this.replan();this.master.gain.linearRampToValueAtTime(this.target(),this.origin+.7);this.timer=window.setInterval(()=>this.schedule(),25);this.schedule()}
  ...
  setCharacter(character:string){
    ...
    this.hold(now);this.master.gain.linearRampToValueAtTime(0,now+.4);
    window.setTimeout(()=>{if(this.stopped||token!==this.generation||next!==this.requestedId)return;for(const source of this.active)try{source.stop()}catch{}this.active.clear();this.id=next;this.cycle=0;this.origin=this.context.currentTime+.06;this.replan();this.master.gain.setValueAtTime(0,this.context.currentTime);this.master.gain.linearRampToValueAtTime(this.target(),this.context.currentTime+.7)},430);
  }
  async stop(){if(this.stopped)return;this.stopped=true;++this.generation;window.clearInterval(this.timer);this.hold(this.context.currentTime);this.master.gain.linearRampToValueAtTime(0,this.context.currentTime+.45);await new Promise(r=>window.setTimeout(r,500));for(const source of this.active)try{source.stop()}catch{}await this.context.close()}
  ...
  private schedule(){
    while(!this.stopped){
      if(this.cursor>=this.timeline.length){if(this.origin+this.timelineDuration>this.context.currentTime+.24)return;this.origin+=this.timelineDuration;++this.cycle;this.replan();continue}
      const event=this.timeline[this.cursor]!,at=this.origin+event.at;if(at>this.context.currentTime+.24)return;this.render(event,at);this.cursor++
    }
  }
  private render(e:MusicEvent,at:number){
    const spec=SCORE_BIBLES[this.id]!.voices[e.role],level=spec.gain*e.gain;
    if(e.noise){this.renderPercussion(e,spec,at,level);return}
    const osc=this.context.createOscillator(),partials=this.partials(spec.instrument),real=new Float32Array(partials.length),imag=new Float32Array(partials.length);
    for(let i=1;i<partials.length;i++)imag[i]=partials[i]!;
    osc.setPeriodicWave(this.context.createPeriodicWave(real,imag,{disableNormalization:false}));
    const frequency=hz(e.midi);osc.frequency.setValueAtTime(frequency,at);
    if(spec.vibrato>0&&e.duration>.35)osc.frequency.setValueCurveAtTime(new Float32Array([frequency,frequency*(1+spec.vibrato),frequency,frequency*(1-spec.vibrato),frequency]),at,e.duration);
    this.wire(osc,spec,at,e.duration,level);
  }
  ...
  private renderPercussion(e:MusicEvent,spec:Voice,at:number,level:number){
    const duration=Math.min(e.duration,spec.instrument==="timpani"||spec.instrument==="taiko"?.38:.16),frames=Math.ceil(this.context.sampleRate*duration),buffer=this.context.createBuffer(1,frames,this.context.sampleRate),data=buffer.getChannelData(0),r=new Random(seedFor(this.baseSeed,this.id+e.role,Math.floor(e.at*1000)+this.cycle));
    for(let i=0;i<frames;i++){const t=i/this.context.sampleRate,decay=Math.exp(-t*(spec.instrument==="taiko"?9:18));data[i]=(r.next()*2-1)*decay*(spec.instrument==="woodblock"?.24:.5)}
    const src=this.context.createBufferSource();src.buffer=buffer;this.wire(src,spec,at,duration,level*.45);
  ...
  private wire(source:AudioScheduledSourceNode,spec:Voice,at:number,duration:number,level:number){
    ...
    const peak=at+Math.min(spec.attack,duration*.3),decayEnd=Math.min(at+duration*.75,peak+spec.decay);
    env.gain.setValueAtTime(.0001,at);env.gain.exponentialRampToValueAtTime(Math.max(.0002,level),peak);env.gain.exponentialRampToValueAtTime(Math.max(.0002,level*spec.sustain),decayEnd);env.gain.setValueAtTime(Math.max(.0002,level*spec.sustain),at+duration);env.gain.exponentialRampToValueAtTime(.0001,at+duration+spec.release);
    source.connect(filter);filter.connect(env);env.connect(pan);pan.connect(this.mix);this.active.add(source);source.addEventListener("ended",()=>this.active.delete(source),{once:true});source.start(at);source.stop(at+duration+spec.release+.03);
  }
}
