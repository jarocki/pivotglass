...
export default function Cockpit() {
  ...
  return <main ref={cockpitRef} tabIndex={-1} style={style} className={`mode-${state?.character ?? "default"} effects-${effects} narration-${narration} display-${displayMode} contrast-${contrast} ${focusView ? "focus-view" : ""} ${maximized ? `max-${maximized}` : ""}`}>
    ...
    <nav className="pane-switcher" aria-label="Primary cockpit panes"><span className="workflow-label">WORKFLOW</span>{paneIds.map((id) => <button key={id} aria-current={activePane === id ? "page" : undefined} title={`Open and focus ${PANE_LABELS[id]} pane`} onClick={() => go(id)}>{PANE_LABELS[id]}</button>)}<button className="focus-toggle" aria-pressed={focusView} title="Show the pursuit workspace with secondary panels tucked away" onClick={() => { const next = !focusView; setFocusView(next); window.localStorage.setItem("pivotglass.focusView", String(next)); }}>{focusView ? "FULL COCKPIT" : "FOCUS VIEW"}</button><button className="utility-control" title="Use the active pane at full viewport size" onClick={() => toggleMaximize(activePane)}>{maximized ? "RESTORE VIEW" : "MAXIMIZE PANE"}</button><button className="utility-control" title="Search all cockpit commands" onClick={(event) => openOverlay("palette", event.currentTarget)}>COMMANDS ⌘K</button><button className="utility-control" title="Configure models and intelligence APIs" onClick={(event) => openOverlay("configuration", event.currentTarget)}>CONFIGURATION</button><button className="utility-control" title="Open optional theme arcade" onClick={(event) => openOverlay("dojo", event.currentTarget)}>THEME ARCADE</button></nav>
