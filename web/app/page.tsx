...
import { FrameworkLenses, type FrameworkState } from "./framework-lenses";
...
type State = { workspace: string; stats: Record<string, number>; objects: EvidenceCard[]; briefings: Record<string, Briefing>; character: string; modes: Mode[]; dossier_slots: DossierSlot[]; visualizations: VisualizationIntent[]; graph_layouts: GraphLayoutSummary[]; analysis: AnalyticSnapshot; frameworks: FrameworkState; processed_targets: string[]; instruments: Instruments; challenges: ChallengeRecord[]; badges: BadgeAward[]; badge_summary: { count: number; latest: BadgeAward | null } };
...
const PANE_LABELS: Record<PaneId, string> = {
  ...
  dossier: "dossier",
  "artifact-field": "charts & evidence",
  systems: "service status",
};
...
export default function Cockpit() {
  ...
  useEffect(() => {
    ...
    const storedFocusView = window.localStorage.getItem("pivotglass.focusView");
    ...
    if (window.localStorage.getItem("pivotglass.uxVersion") !== "0.8.5") {
      panes["artifact-field"] = true;
      window.localStorage.setItem("pivotglass.uxVersion", "0.8.5");
      window.localStorage.setItem("pivotglass.panes", JSON.stringify({ ...collapsed, ...panes }));
    }
    setCollapsed((current) => ({ ...current, ...panes }));
    setMusic(window.localStorage.getItem("pivotglass.music.enabled") === "true");
    setFocusView(storedFocusView === "true");
  ...
  function go(id: PaneId) { if (focusView && id !== "intelligence") { setFocusView(false); window.localStorage.setItem("pivotglass.focusView", "false"); } setActivePane(id); if (id === "dossier" || id === "artifact-field") setInspectorOpen(true); if (collapsed[id]) togglePane(id); const url = new URL(window.location.href); url.searchParams.set("pane", id); window.history.pushState({ pane: id }, "", url); requestAnimationFrame(() => { const panel = document.getElementById(id); panel?.scrollIntoView({ behavior: effects === "full" ? "smooth" : "auto", block: "center" }); panel?.focus({ preventScroll: true }); }); }
  ...
  return <main ref={cockpitRef} tabIndex={-1} style={style} className={`mode-${state?.character ?? "default"} effects-${effects} narration-${narration} display-${displayMode} contrast-${contrast} ${focusView ? "focus-view" : ""} ${maximized ? `max-${maximized}` : ""}`}>
    ...
    <nav className="pane-switcher" aria-label="Primary cockpit panes"><span className="workflow-label">WORKFLOW</span>{paneIds.map((id) => <button key={id} aria-current={activePane === id ? "page" : undefined} title={`Open and focus ${PANE_LABELS[id]} pane`} onClick={() => go(id)}>{PANE_LABELS[id]}</button>)}<button className="focus-toggle" aria-pressed={focusView} title="Show the pursuit workspace with secondary panels tucked away" onClick={() => { const next = !focusView; setFocusView(next); window.localStorage.setItem("pivotglass.focusView", String(next)); }}>{focusView ? "FULL COCKPIT" : "FOCUS VIEW"}</button><button className="inspector-toggle" aria-pressed={inspectorOpen} title="Open the contextual dossier inspector" onClick={() => setInspectorOpen((current) => !current)}>{inspectorOpen ? "HIDE INSPECTOR" : "INSPECTOR"}</button><button className="utility-control" title="Open service status, attention records, and display utilities" onClick={() => setUtilityOpen(true)}>UTILITIES{alerts.unread_count ? ` · ${alerts.unread_count}` : ""}</button><button className="utility-control" title="Use the active pane at full viewport size" onClick={() => toggleMaximize(activePane)}>{maximized ? "RESTORE VIEW" : "MAXIMIZE PANE"}</button><button className="utility-control" title="Search all cockpit commands" onClick={(event) => openOverlay("palette", event.currentTarget)}>COMMANDS ⌘K</button><button className="utility-control" title="Configure models and intelligence APIs" onClick={(event) => openOverlay("configuration", event.currentTarget)}>CONFIGURATION</button><button className="utility-control" title="Open optional theme arcade" onClick={(event) => openOverlay("dojo", event.currentTarget)}>THEME ARCADE</button></nav>
    ...
    <section className={`telemetry-rack ${collapsed.systems ? "collapsed" : ""}`} id="systems" tabIndex={-1}>
      ...
      <Meter label="DOSSIER COVERAGE" value={(dossierProgress / 9) * 100} detail={`${dossier} FILLED`} warning={dossier >= 8} />
      <Meter label="ENRICHMENT SOURCES" value={null} detail={`${configuredSources} CONFIGURED`} />
      <Meter label="MODEL CAPACITY" value={state?.instruments.model_tokens.available ? 100 : null} detail={state?.instruments.model_tokens.available ? `${state.instruments.model_tokens.used ?? 0} USED` : "NOT ENGAGED"} />
      <div className="damage"><span>INVESTIGATION STATUS</span><b className={error ? "danger" : active ? "caution" : "ok-text"}>{error ? "FAULT" : active ? "RUNNING" : "IDLE"}</b><small>{error ? "SEE ATTENTION NEEDED" : active ? `${elapsed}s ELAPSED · EVENT STREAM LIVE` : "NO ACTIVE INVESTIGATION"}</small></div></div>
    </section>

    <section className="cockpit-grid">
      <article className={`panel feed-panel ${collapsed.intelligence ? "collapsed" : ""}`} id="intelligence" tabIndex={-1}><PanelTitle id="intelligence" title={`${mode?.cockpit.left_rail} ${mode?.pursuit_title} // PURSUIT`} status={`${taskGroups.length} TASKS · ${feed.length} EVENTS`} collapsed={collapsed.intelligence} maximized={maximized === "intelligence"} onToggle={() => togglePane("intelligence")} onMaximize={() => toggleMaximize("intelligence")}/><div id="intelligence-content" className="feed" ref={feedRef} tabIndex={0} aria-label="Scrollable intelligence feed" onScroll={(event) => { const node = event.currentTarget; setReviewingHistory(node.scrollHeight - node.scrollTop - node.clientHeight > 32); }}>
        {state?.analysis && <ScientificWorkbench analysis={state.analysis} onCommand={runAnalyticCommand}/>}
        {state?.frameworks && <FrameworkLenses frameworks={state.frameworks} onCommand={(command) => void runQuick(command)}/>}
